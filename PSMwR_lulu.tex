...
\NormalTok{ls <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"rgdal"}\NormalTok{, }\StringTok{"raster"}\NormalTok{, }\StringTok{"sf"}\NormalTok{, }\StringTok{"GSIF"}\NormalTok{, }\StringTok{"plotKML"}\NormalTok{, }
        ...
        \StringTok{"quantregForest"}\NormalTok{, }\StringTok{"intamap"}\NormalTok{, }\StringTok{"fasterize"}\NormalTok{)}
...
\caption{Example of a soil water content plot. Actual water content can be measured using soil moisture probes i.e. automated sensor networks.}\label{fig:available-soil-water}
\end{figure}
...
The resulting object now contains also slots of type \texttt{labpt} which is exactly the geometric gravity point of the first polygon automatically derived by the \texttt{SpatialPolygons} function.
...
\caption{Ebergotzen zones rasterized to 25 m resolution.}\label{fig:eberg-zones-rasterized}
\end{figure}
...
\CommentTok{#> R 2019-02-03 18:23:46 interpolating 155 observations, 3103 prediction locations}
...
\CommentTok{#> [1] "estimated time for  copula 159.339745652053"}
...
\CommentTok{#>   .. ..- attr(*, ".Environment")=<environment: 0x14e79380> }
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/bwplot-meuse-1} 
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/z-scores-histogram-1} 
...
{\centering \includegraphics[width=0.9\linewidth]{PSMwR_lulu_files/figure-latex/confidence-limits-boxplot-1} 
...
{\centering \includegraphics[width=0.7\linewidth]{figures/Measured_vs_predicted_SAND_plot} 
...
\CommentTok{#> Model ID:  DeepLearning_model_R_1549218286903_2 }
...
\CommentTok{#> 2     2   200 Rectifier  0.00 % 0.000000 0.000000  0.014974 0.009187}
\CommentTok{#> 3     3   200 Rectifier  0.00 % 0.000000 0.000000  0.148762 0.188431}
\CommentTok{#> 4     4     1    Linear      NA 0.000000 0.000000  0.001476 0.001069}
\CommentTok{#>   momentum mean_weight weight_rms mean_bias bias_rms}
\CommentTok{#> 1       NA          NA         NA        NA       NA}
\CommentTok{#> 2 0.000000   -0.001359   0.104054  0.353022 0.065334}
\CommentTok{#> 3 0.000000   -0.018556   0.071409  0.952353 0.019921}
\CommentTok{#> 4 0.000000   -0.001455   0.047038  0.100393 0.000000}
\CommentTok{#> }
\CommentTok{#> }
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/map-snd-ensemble-1} 
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/maps-soc-edgeroi-1} 
...
\CommentTok{#>   ..$ Predicted: num [1:4972] 15.21 10.32 9.09 5.96 3.42 ...}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plt0 <-}\StringTok{ }\KeywordTok{xyplot}\NormalTok{(test.ORC[[}\DecValTok{1}\NormalTok{]]}\OperatorTok{$}\NormalTok{Observed }\OperatorTok{~}\StringTok{ }\NormalTok{test.ORC[[}\DecValTok{1}\NormalTok{]]}\OperatorTok{$}\NormalTok{Predicted, }\DataTypeTok{asp=}\DecValTok{1}\NormalTok{, }\DataTypeTok{par.settings=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{plot.symbol =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{col=}\NormalTok{scales}\OperatorTok{::}\KeywordTok{alpha}\NormalTok{(}\StringTok{"black"}\NormalTok{, }\FloatTok{0.6}\NormalTok{), }\DataTypeTok{fill=}\NormalTok{scales}\OperatorTok{::}\KeywordTok{alpha}\NormalTok{(}\StringTok{"red"}\NormalTok{, }\FloatTok{0.6}\NormalTok{), }\DataTypeTok{pch=}\DecValTok{21}\NormalTok{, }\DataTypeTok{cex=}\FloatTok{0.6}\NormalTok{)), }\DataTypeTok{scales=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{x=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{log=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{equispaced.log=}\OtherTok{FALSE}\NormalTok{), }\DataTypeTok{y=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{log=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{equispaced.log=}\OtherTok{FALSE}\NormalTok{)), }\DataTypeTok{ylab=}\StringTok{"measured"}\NormalTok{, }\DataTypeTok{xlab=}\StringTok{"predicted (machine learning)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics[width=0.7\linewidth]{figures/Predicted_vs_observed_plot_for_SOC_edgeroi} 
...
\CommentTok{#> 1 h2o.randomForest.wrapper 8.44}
\CommentTok{#> }
\CommentTok{#> }
...
\CommentTok{#> Family: gaussian}
\CommentTok{#> }
\CommentTok{#> Ensemble performance (MSE): 8.31668075952835}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> 4 h2o.deeplearning.wrapper 0.1411}
...
\CommentTok{#> Family: gaussian}
\CommentTok{#> }
\CommentTok{#> Ensemble performance (MSE): 0.0806321502420654}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> SL.xgboost_All 0.0598 0.8200}
\CommentTok{#> SL.ksvm_All    0.1290 0.0163}
\CommentTok{#> SL.glmnet_All  0.3077 0.0000}
\CommentTok{#> SL.ranger_All  0.0855 0.1637}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> SL.ranger_All  0.0831 0.185}
\CommentTok{#> SL.ksvm_All    0.1311 0.000}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#>   Super Learner 0.16 0.014 0.097 0.25}
\CommentTok{#>     Discrete SL 0.17 0.015 0.117 0.25}
...
\CommentTok{#> List of 2}
\CommentTok{#>  $ pred           : num [1:3865, 1] 4.64 4.7 4.85 4.83 4.76 ...}
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/ph-cookfarm-var-1} 
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/rf-variableImportance-1} 
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/plot-cosha30map-rf-1} 
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/plot-edgeroi-ocd-1} 
...
{\centering \includegraphics[width=1\linewidth]{PSMwR_lulu_files/figure-latex/thist-usa48-1} 
