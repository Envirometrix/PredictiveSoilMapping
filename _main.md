...
conventional soil inventories framed within the context of PSM. We identify soils,
their associated properties, and their spatial distribution as the
...
Soil is the Earth’s biggest carbon store containing of total terrestrial
...
describe reality one would need a model at 1:1 scale at which of reality
was represented by of the model. Since this is not feasible, we condense
...
model of soil variation can often approach, or even exceed, 30– of the
...
-   *spatial support (resolution) of covariate layers*: ;

-   *spatial support of predictions*: point support (centre of a grid
    cell);

-   *amount of variation explained by the spatial prediction model*: ;
...
-   *Soil field observations and measurements* — Observations and
    ...
    (for example *‘ south of village A’*), or might even
...
of soil having a limited horizonal extent of no more than 1– in
horizontal and a vertical dimension ($d$) that typically extends to only
1–2 but may occasionally extend to greater depths. In practice, the vast
majority of soil profile data pertain to soil observations and samples
collected over very limited horizontal dimensions (10 m) and down to
...
varying support sizes for which composite sampling can be used.
...
few meters to up to surrounding the location where the vertical profile
...
an accuracy of better than 50 m. Some widely used profile data from
developing countries cannot be reliably located to within few hundred meters [@Leenaars2012].
...
from 50 m. If the location of a point soil profile is only known
with an accuracy of 50 m, then, when overlaid on topographic data, that
...
smaller when the accuracy of the point location is $\pm$100 m. In such cases,
...
explain over of the fitted model, but these are unknown until we fit the
...
variable soil properties, as much as 40– of the total variation in that
...
portion of the variation (40–) that occurs over longer distances
...
[@Rossiter2004SUM; @Hartemink2008SMD]. Most smaller scale (1:1 or
...
imagine if the location accuracy for soil profiles is $\pm$1000 m or poorer.
...
significant portion of uncertainty. A map can never be 100% accurate
[@Oreskes04021994].
...
quantity of resolution maps) to map the distribution of soil organic
...
extensive modeling, they were able to explain only an average of of the
...
short-range variability, which is unmappable at resolution, that we will
...
explain at least of the original variance
...
model that explains of variability, which would be fairly difficult to
achieve.
...
The R tutorial on the end of this chaper reviews soil data classes and 
...
field and following some soil description guidelines. Examples of
...
*‘target’* (i.e. targeted soil properties) and *‘paths’* i.e.
...
observed effective rooting depth of a given soil at a given site.
...
following soil profile from Nigeria [@leenaars2012africa]:
...
cotton can still extract water at (pF 4.4) while pepper wilts at (pF
...
prediction methods is in fact not large ($\pm5$% of variance explained).
...
variables. The authors of the [^3] package for , for example, have
...
or $6.3\pm 2.1$ where half the error of estimating organic matter 
...
precision of $\pm0.54$! This number means that, if we would like to
estimate (aggregated) mean value for the whole population then the
standard error of that mean would be $\pm0.54$. In other words the
population mean for organic matter based on 153 samples is
$7.48\pm 0.54$, but if we would know the values of organic matter at
individual locations, then the confidence limits are about $7.48\pm 3.4$
