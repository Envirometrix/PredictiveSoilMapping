...
```{block, type="rmdnote"}
The most common output of soil resource inventories are *soil maps*. Soil maps convey information
...
objective of soil mapping is to accurately predict response of a
...
```{r soil-crop-model-scheme, echo=FALSE, fig.cap="Inputs to soil-plant, soil-hydrology or soil-ecology models and their relationship.", out.width="100%"}
...
```{r comparison-dsm, echo=FALSE, fig.cap="Comparison between traditional (primarily expert-based) and automated (data-driven) soil mapping.", out.width="100%"}
...
of PSM is to produce optimal unbiased predictions of mean value at some new location along with the uncertainty associated with the prediction, at the finest possible resolution.
```
...
```{r soilsurvey-scheme, echo=FALSE, fig.cap="Typical soil survey phases and intermediate and final products.", out.width="100%"}
...
```{r soilmap-types, echo=FALSE, fig.cap="Classification of types of soil maps based on spatial representation and variable type.", out.width="100%"}
...
```{r new-technologies, echo=FALSE, fig.cap="Evolution of digital soil mapping parallels the emergence of new technologies and global, publicly available data sources.", out.width="100%"}
...
```{block, type="rmdnote"}
Differences among *conventional soil mapping*, *digital soil mapping* or
...
(Fig. \@ref(fig:pedometric-mapping-vs-dsm)).
```
...
```{r pedometric-mapping-vs-dsm, echo=FALSE, fig.cap="A classification of approaches to soil mapping: from purely expert driven, to various types of *digital* soil mapping.", out.width="85%"}
...
```{r dem-evolution, echo=FALSE, fig.cap="Evolution of global DEM data sources: (right) SRTM DEM released in 2002, as compared to (left) WorldDEM released in 2014 [@Baade2014IEE]. Sample data set for city of Quorn in South Australia. As with many digital technologies, level of detail and accuracy of the GIS and remote sensing data is following an exponential growth.", out.width="100%"}
knitr::include_graphics("figures/Fig_DEM_evolution.png")
```

```{block, type="rmdnote"}
Most common soil covariates used in soil mapping
are: (1) Raw spectral and multi-spectral images of the land surface, (2)
DEM-derivates, (3) Climatic images, (4) Vegetation and land-cover based
covariates, (5) Land survey and land use information, and (6)
Expert-based covariates e.g. soil delineations.
```
...
```{r from-photointerpretation-to-soilmap, echo=FALSE, fig.cap="In conventional soil mapping, soil delineations are usually manually drawn polygons representing (assumed) bodies of homogenous soil materials (often geomorphological units). These are first validated on the field until the final area-class map is produced, which can then be generalized and used to extract soil property maps. After @SSS1983USDA.", out.width="95%"}
knitr::include_graphics("figures/Fig_from_photointerpretation_to_soilmap.png")
```
...
accuracy of thematic content, as well as from poor positional accuracy.
```
...
```{r site-horizon-structure, echo=FALSE, fig.cap="An example of a basic soil profile geographical database, which commonly consists of four tables: SITE, HORIZON, DESCRIPTION and NAMES tables (a). To facilitate rapid display and use of soil variables, SITE and HORIZON tables can be combined into a single (*wide*) table structure (b).", out.width="80%"}
knitr::include_graphics("figures/Fig_SITE_HORIZON_structure.png")
```
...
mapping — a standardized collection of (gridded) soil property and class
...
biodiversity assessment and similar. A SIS offers the best possible soil
...
```{r automap-future, echo=FALSE, fig.cap="The future of global mapping and environmental monitoring activities is expected to be increasingly automated and distributed.", out.width="75%"}
knitr::include_graphics("figures/Fig_automap_future.png")
```
...
the global value of soil for fixation of gasses and water holding?
```
...
```{r usability-scheme, echo=FALSE, fig.cap="Usability of a Soil Information System is basically a function of a number of data usability measures from which the following four (C’s) are essential: consistency, completeness, correctness and currency.", out.width="95%"}
knitr::include_graphics("figures/Fig_usability_scheme.png")
```
...
```{block, type="rmdnote"}
Quality of a geospatial database is a function
...
be optimized up to a certain level depending on the available funds.
```
...
extension to soil mapping outputs. Maps of uncertainty can be submitted
...
```{r lines-legros1997, echo=FALSE, fig.cap="20 photo-interpretations done independently using the same aerial photograph overlaid on top of each other. Image credit: @Legros1997ESG.", out.width="50%"}
knitr::include_graphics("figures/Fig_lines_Legros1997.png")
```
...
quantified either via the geostatistical model, or by using the
...
```{r lagacherie1992, echo=FALSE, fig.cap="Reduction of prediction error as a function of sampling intensity (for three control areas). Based on @Lagacherie1992PhD.", out.width="85%"}
knitr::include_graphics("figures/Fig_Lagacherie1992.png")
```
...
```{r sigma-rmse-relationship, echo=FALSE, fig.cap="Relationship between the numeric resolution (visualized using a histogram plot on the left) and amount of variation explained by the model for soil pH.", out.width="100%"}
knitr::include_graphics("figures/Fig_sigma_RMSE_relationship.png")
```
